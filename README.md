# Debate Network

## English

DebateNetwork is a web-based project that provides access to a variety of religious texts, including the Quran, and Hadith, and Bible, with search and comparison features. The project is organized into several sections, each dedicated to a specific text or resource, and includes scripts and data files for enhanced interactivity.

### Features
- Quran, Bible, and Hadith texts with search and comparison tools
- Organized folder structure for easy navigation
- JSON data files for fast access and processing
- Simple and clean user interface

### Folder Structure
- `bible/` — Bible texts, search, and comparison
- `quran/` — Quranic texts, search, and comparison
- `hadith/` — Hadith texts, search, and comparison
- `blog/` — Blog articles and topics
- `about/` — About, contact, and FAQ pages
- `font/`, `img/` — Fonts and images

### Usage
Open `index.html` in your browser to start using the application. Navigate through the folders for specific resources.

### Link: nbras002.github.io/DebateNetwork or debatenetwork.onrender.com

* If the link does not work, use a local server or a public server.

---

## العربية

مشروع DebateNetwork هو مشروع ويب يتيح الوصول إلى مجموعة متنوعة من النصوص الدينية، بما في ذلك القرآن الكريم والحديث الشريف والكتاب المقدس، مع ميزات البحث والمقارنة. المشروع منظم في عدة أقسام، كل قسم مخصص لنص أو مصدر معين، ويحتوي على سكريبتات وملفات بيانات لزيادة التفاعل.

### الميزات
- نصوص القرآن الكريم والكتاب المقدس والحديث مع أدوات البحث والمقارنة
- هيكل مجلدات منظم لسهولة التصفح
- ملفات بيانات JSON للوصول والمعالجة السريعة
- واجهة مستخدم بسيطة وواضحة

### هيكل المجلدات
- `bible/` — نصوص الكتاب المقدس والبحث والمقارنة
- `quran/` — نصوص القرآن الكريم والبحث والمقارنة
- `hadith/` — نصوص الحديث الشريف والبحث والمقارنة
- `blog/` — مقالات ومواضيع المدونة
- `about/` — صفحات حول الموقع والتواصل والأسئلة الشائعة
- `font/`, `img/` — الخطوط والصور

### طريقة الاستخدام
افتح ملف `index.html` في المتصفح لبدء استخدام التطبيق. يمكنك التنقل بين المجلدات للوصول إلى الموارد المختلفة.



### الرابط: https://nbras002.github.io/DebateNetwork أو https://debatenetwork.onrender.com

* إذا لم يعمل الرابط استخدم خادم محلي أو خادم عام.

